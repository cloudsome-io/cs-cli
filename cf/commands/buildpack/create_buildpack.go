...
func (cmd CreateBuildpack) createBuildpack(buildpackName string, c flags.FlagContext) (buildpack models.Buildpack, apiErr error) {
	...
	if err != nil {
		apiErr = fmt.Errorf(T("Error {{.ErrorDescription}} is being passed in as the argument for 'Position' but 'Position' requires an integer.  For more syntax help, see `{{.Command}} -h`.", map[string]interface{}{
			"ErrorDescription": c.Args()[2],
			"Command":          cf.Name + " create-buildpack",
		}))
		return
	}
